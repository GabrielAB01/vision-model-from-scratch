...
class PaliGemmaProcessor:
	"""
	Processor for the PaliGemma model.
	Args:
		tokenizer: The tokenizer to use for text processing.
		num_image_tokens: The number of image tokens to prepend to the text.
		image_size: The size of the images to process."""
	...
	def __call__(
		...
		Args:
			text (List[str]): The text prompts to process.
			images (List[Image.Image]): The images to process.
			padding (str): The padding strategy to use. Defaults to "longest".
			truncation (bool): Whether to truncate the text. Defaults to True.
		Returns:
			Dict: A dictionary containing the processed pixel values, input IDs, and attention masks.
		"""
		# Check if the number of images and text prompts are equal
		if len(images) != len(text):
			raise ValueError(
				f"Number of images ({len(images)}) must match number of text prompts ({len(text)})."
			)
